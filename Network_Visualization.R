...
# This RScript visualizes phonetic distances between languages using network
# by transforming the data file generated in 'Phonetic_Distance.R' into
# an interactive network map.
# Nodes represent languages, and edges represent phonetic similarity (overlap). 
# Users can search for languages, highlight connected nodes, and generate sub-networks.
...
# Data Loading
# Load phonetic distances (replace with your actual file path)
file_phonetic_distances <- "[phonetic_distances.csv Pathname]"                 
  df.phonetic.distances <- read.csv(file_phonetic_distances)

# Load language names (replace with your actual file path)
file_languages <-   #"[languages.csv Pathname]"
df.languages <- read.csv(file_languages)
df.language.names <- df.languages[, 1:2]

# Data Preparation
# Set minimum edge width (adjust as needed)
min_edge_width <- 0.34

# Filter edges based on minimum width
df.phonetic.distances.trimmed <- subset(df.phonetic.distances, 
                                        df.phonetic.distances$overlap 
                                        >= min_edge_width)

# Graph Creation and Calculations
# Create graph object from filtered data
g <- graph_from_data_frame(df.phonetic.distances.trimmed[, c("lang1", "lang2")])

# Assign edge weights
E(g)$weight <- as.numeric(df.phonetic.distances.trimmed$overlap) + 0.01
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      actionButton("generateFromSelected", "Generate from Selected"),  # New button
    ...
    if (!is.null(selected_node())) {
      ...
      visNetworkProxy("network") %>%
        visUpdateNodes(nodes = data.frame(
          ...
          size = ifelse(nodes$id %in% highlighted_node_ids, nodes$size * 2, nodes$size)# Purple for connected nodes
        )) %>%
        visUpdateEdges(edges = data.frame(
          id = edges$id,
          color = ifelse(as.numeric(edges$id) %in% highlighted_node_ids, "purple", "gray"),
          size = ifelse(edges$id %in% highlighted_node_ids, nodes$size * 2, nodes$size)  # Double size for purple nodes
        ))
    } else {
    ...
    observeEvent(c(search_nodes()), {
      ...
      visNetworkProxy("network") %>%
        visUpdateNodes(nodes = data.frame(
          ...
          size = ifelse(nodes$id %in% search_node_ids, nodes$size * 2, nodes$size))  # Double size for red nodes)
        )
    }
    )
  }
